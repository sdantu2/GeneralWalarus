...
class SSESession:
    """ Class that encapsulates a details about a Srinath Stock Exchange session """
    ...
    def __init__(self, guild: Guild, cron_exp: str) -> None:
        ...
        self.job: Job = scheduler.add_job(self.__get_new_sse_price, cron_trigger, args=[True])
        """ APScheduler job used for fetching next price change timestamp """
        scheduler.start()

    def __str__(self) -> str:
        curr_price = db.get_current_sse_price(self.guild)
        return (f"'{self.guild.name}': ${curr_price} (next price change: {self.job.next_run_time})")
    
    def __get_new_sse_price(self, write: bool = False) -> float:
